                ...
                for (GameUnit gameUnit : listOfSowEnemys) {
                    ...
                    for (Trooper trooper1 : troopers) {
                        ...
                        if (trooper1.isTeammate() && world.isVisible(trooper1.getVisionRange(), trooper1.getX(), trooper1.getY(), trooper1.getStance(), gameUnit.trooper.getX(), gameUnit.trooper.getY(), TrooperStance.PRONE)) {
                            isOldOutdated = true;
                            break;
                        } else if (trooper1.isTeammate() && !world.isVisible(trooper1.getVisionRange(), trooper1.getX(), trooper1.getY(), trooper1.getStance(), gameUnit.trooper.getX(), gameUnit.trooper.getY(), TrooperStance.PRONE) && self.getVisionRange() >= self.getDistanceTo(gameUnit.trooper)) {
                            countSee++;
    ...
    void moveToTarget(Trooper self, int targetX, int targetY) {
        ...
        if (self.getActionPoints() >= game.getStanceChangeCost()) {
            ...
            if (detectEnemyByTeam && listOfEnemyTroopers.size() == 0 && targetY == localTargetY && targetX == localTargetX/* && !world.isVisible(self.getVisionRange() - 1, self.getX(), self.getY(), self.getStance(), localTargetX, localTargetY, TrooperStance.PRONE)*/) {
                ...
                if (path != null && path.size() > 4 && path.size() < path1.size() + 5 && trueMapOfPoints[path.get(1).getX()][path.get(1).getY()] > 2 && self.getActionPoints() >= 6/* && !world.isVisible(self.getVisionRange() - 1, self.getX(), self.getY(), self.getStance(), localTargetX, localTargetY, TrooperStance.PRONE)*/) {
                    if (goOnPath(self, path.get(1).getX(), path.get(1).getY(), true)) {
                        return;
                    }
                } else if (path1 != null && path1.size() > 4 && trueMapOfPoints[path1.get(1).getX()][path1.get(1).getY()] > 2 && self.getActionPoints() >= 6/* && !world.isVisible(self.getVisionRange() - 1, self.getX(), self.getY(), self.getStance(), localTargetX, localTargetY, TrooperStance.PRONE)*/) {
                    if (goOnPath(self, path1.get(1).getX(), path1.get(1).getY(), false)) {
                        return;
                    }
                }

                if (path != null && path.size() > 4 && path.size() < path1.size() + 5 && trueMapOfPoints[path.get(1).getX()][path.get(1).getY()] < 4 && self.getActionPoints() >= 6/* && !world.isVisible(self.getVisionRange() - 1, self.getX(), self.getY(), self.getStance(), localTargetX, localTargetY, TrooperStance.PRONE)*/) {
                    if (goOnPath(self, path.get(1).getX(), path.get(1).getY(), true)) {
                        return;
                    }
                } else if (path1 != null && path1.size() > 4 && trueMapOfPoints[path1.get(1).getX()][path1.get(1).getY()] < 4 && self.getActionPoints() >= 6/* && !world.isVisible(self.getVisionRange() - 1, self.getX(), self.getY(), self.getStance(), localTargetX, localTargetY, TrooperStance.PRONE)*/) {
                    if (goOnPath(self, path1.get(1).getX(), path1.get(1).getY(), false)) {
                        return;
                    }
                }

                if (path != null && path.size() > 1 && path.size() < path1.size() + 5 && trueMapOfPoints[path.get(0).getX()][path.get(0).getY()] < 4 && self.getActionPoints() >= 6/* && !world.isVisible(self.getVisionRange() - 1, self.getX(), self.getY(), self.getStance(), localTargetX, localTargetY, TrooperStance.PRONE)*/) {
                    if (goOnPath(self, path.get(1).getX(), path.get(1).getY(), true)) {
                        return;
                    }
                } else if (path1 != null && path1.size() > 1 && trueMapOfPoints[path1.get(0).getX()][path1.get(0).getY()] < 4 && self.getActionPoints() >= 6/* && !world.isVisible(self.getVisionRange() - 1, self.getX(), self.getY(), self.getStance(), localTargetX, localTargetY, TrooperStance.PRONE)*/) {
                    if (goOnPath(self, path1.get(1).getX(), path1.get(1).getY(), false)) {
                        return;
                    }
                }
        ...
        if (self.getActionPoints() >= getCostMoveWithStance(self)) {
            ...
            if (goToSafePlace) {
                ...
                if(safePoint != null) {
                    ...
                    if (targetX == self.getX() && targetY == self.getY()) {
                        if (self.getActionPoints() >= game.getStanceChangeCost() && (self.getStance() == TrooperStance.STANDING && safeStance == TrooperStance.KNEELING || self.getStance() == TrooperStance.KNEELING && safeStance == TrooperStance.PRONE)) {
                            goToSafePlace = false;
                            safePoint = null;
                            move.setAction(ActionType.LOWER_STANCE);
...
//                                move.setAction(ActionType.END_TURN);
    ...
    thePoint findNotAchievableTail(Trooper self, boolean seeOrShoot, int additionalRange) {
        ...
        if(safeStance == null && !(istroopersUnderAttack && trooperUnderAttack == self.getId())) {
            ...
            for (Trooper trooper : listOfEnemyTroopers) {
                if (self.getActionPoints() >= 4 && self.getStance() == TrooperStance.STANDING && world.isVisible(trooper.getVisionRange() + additionalRange, trooper.getX(), trooper.getY(), TrooperStance.STANDING, self.getX(), self.getY(), TrooperStance.PRONE)) {
        ...
        for (int k = 0; k < W; k++) {
            for (int m = 0; m < H; m++) {
                ...
                if (isNotStoredCell && cellsIntTemp[k][m] == BLANK && self.getDistanceTo(k, m) < moveLen) {
                    LinkedList<thePoint> tempPath = lee(self, self.getX(), self.getY(), k, m, true);
                    for (Trooper trooper : troopers) {
                        if (trooper.getX() == k && trooper.getY() == m) {
                            tempPath = null;
                    ...
                    if (targetHeal != null && self.getDistanceTo(targetHeal) <= 1) {
                        ...
                        for (Trooper trooper : listOfEnemyTroopers) {
                            ...
                            if (self.getDistanceTo(trooper) <= self.getVisionRange() && tempPath != null && tempPath.size() > 1 && (tempPath.size() - 1) * game.getStandingMoveCost() > actionPoints) {
                                ...
                                if ( !(actionPoints - 4 >= game.getStandingMoveCost() && !world.isVisible(trooper.getVisionRange(), trooper.getX(), trooper.getY(), TrooperStance.STANDING, tempPath.get(actionPoints / 2).getX(), tempPath.get(actionPoints / 2).getY(), TrooperStance.KNEELING) || actionPoints - 4 >= game.getStandingMoveCost() && !world.isVisible(trooper.getVisionRange(), trooper.getX(), trooper.getY(), TrooperStance.STANDING, tempPath.get(actionPoints / 2).getX(), tempPath.get(actionPoints / 2).getY(), TrooperStance.PRONE))) {
                                    canMove = false;
                                    break;
                                }
                            }
                        ...
                        //медик идёт к targetHeal и если враги есть вокруг, то ложиться
                        actionPoints -= goDownStance;
...
//      Обнуление локальной цели при её достижимости
    ...
    boolean underAttack(Trooper self) {
        if (istroopersUnderAttack && trooperUnderAttack == self.getId() && listOfEnemyTroopers.size() == 0) {
            ...
            //ничего не делаем, идём дальше, в goOnWar учтено, что юнит под атакой
        }
        return false;
    }
            ...
            if (tempPath != null && tempPath.size() > 1 && (tempPath.size() - 1) * getCostMoveWithStance(self) + self.getShootCost() <= self.getActionPoints()) {
                ...
                if (canShootOnTarget(self, target)) {
                    ...
                    if (self.getType() == TrooperType.SNIPER) {
                        ...
                        if (tempPath1!= null && tempPath1.size() > 1 && self.getActionPoints() - goUpStance - (tempPath1.size() - 1) * 2 - 4 >= 0) {
                            if (goOnPath(self, target.getX(), target.getY(), true)) {
                                return true;
                            }
                        }
                    ...
                    if (self.getActionPoints() - goUpStance - (tempPath.size() - 1) * getCostMoveWithStance(self) >= 4) {
                        if (goOnPath(self, target.getX(), target.getY(), true)) {
                            return true;
                        }
                    }
    ...
    /*//TODO надо ли это?
    boolean testFunc (Trooper self) {

        cellsInt[self.getX()][self.getY()] = -5;
        thePoint point = findNotAchievableTail(self, true, 0);
        ...
        LinkedList<thePoint> badPoints = new LinkedList<thePoint>();

        do {
            point = freeCell(self.getX(), self.getY(), badPoints);
            boolean isBadPoint = false;
            for (Trooper trooper : troopers) {
                if (trooper.getX() == point.getX() && trooper.getY() == point.getY()) {
                    isBadPoint = true;
                    break;
                }
            }
    ...
    boolean testOnTrueMap(Trooper self, LinkedList<MyStrategy.thePoint> path1) {
        if (self.getActionPoints() < (clearSelfArea(self, 1) ? game.getStandingMoveCost() * 3 : game.getStandingMoveCost() * 4)) {
            ...
            if (path1 != null && path1.size() > 1 && ((trueMapOfPoints[path1.get(1).getX()][path1.get(1).getY()] < 4) || trueMapOfPoints[self.getX()][self.getY()] > trueMapOfPoints[path1.get(1).getX()][path1.get(1).getY()])) {
                isUseLastMove = false;
                move.setAction(ActionType.END_TURN);
                return true;
            }
